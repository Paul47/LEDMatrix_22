...
#define SPEED  50       //1=fast. 20- = moderate, 50 = slow
...
void colorwaves( CRGB* ledarray, uint16_t numleds, CRGBPalette16& palette) 
{
  ...
  for( uint16_t i = 0 ; i < end; i++) {
    ...
    uint8_t node = random8(0, 2);       //needed to fill entire screen
    int16_t pixelnumber= i;

    for (int y = 0; y < MATRIX_WIDTH; y++) {
        pixelnumber = (MATRIX_WIDTH - y/node) - pixelnumber;
        for (int x = 0; x < MATRIX_WIDTH; x++) {
            // nblend( ledarray[pixelnumber], newcolor, 128);
            nblend(ledarray[leds.mXY(pixelnumber, y)], newcolor, 128);
